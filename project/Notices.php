...
$name= $_SESSION['name'];
	...
	$id++;
	while(1)
	{$sql2= "select * from notice where notice_id='Notice{$id}'";
	...
	$sql1="insert into notice values ('Notice{$id}','{$type}', '{$name}','{$notice}',1)";
...
<td>			
			...
			<?php
			$sqlverify="select * from notice where status=0";
...
<?php }?>
		...
		<table>	
		<tr>
		<td colspan="2" align = "right"> <a href="AdminHome.php" > Go Home  </a> <div id = "d"> <?php echo $val; ?> </div></td>
		</tr>
		 </table>
